// client/src/components/templates/Template3.jsx
import React, { useState, useEffect } from 'react';

const Template3 = ({ portfolioData }) => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white bg-opacity-90 backdrop-blur-sm shadow-md">
        <div className="container mx-auto px-6">
          <div className="flex items-center justify-between h-16">
            <span className="text-2xl font-bold text-purple-600">{personalInfo.name.split(' ')[0]}</span>
      ...
      {/* Hero Section */}
      <section id="home" className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-500 via-pink-500 to-red-500 pt-16">
        <div className="absolute inset-0 bg-black opacity-10"></div>
        <div className="relative text-center text-white space-y-6 p-6 max-w-4xl mx-auto">
          <h1 className="text-6xl md:text-7xl font-bold mb-4 animate-fade-in">
            {personalInfo.name}
          </h1>
          <h2 className="text-3xl md:text-4xl font-light mb-8">{personalInfo.title}</h2>
          <p className="text-xl md:text-2xl leading-relaxed mb-12">{personalInfo.bio}</p>
          
          <div className="flex flex-wrap justify-center gap-6 mt-12">
            {personalInfo.contact.email && (
              <a 
                href={`mailto:${personalInfo.contact.email}`}
                className="bg-white text-purple-600 px-8 py-3 rounded-full hover:bg-opacity-90 transition-all transform hover:scale-105"
              >
                Contact Me
              </a>
            )}
            {personalInfo.contact.linkedin && (
              <a 
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="bg-transparent border-2 border-white text-white px-8 py-3 rounded-full hover:bg-white hover:text-purple-600 transition-all transform hover:scale-105"
              >
                LinkedIn
              </a>
            )}
                    ...
                    <div className="rounded-xl overflow-hidden shadow-2xl">
                      {project.media[0].type === 'image' ? (
                        ...
                        <div className="w-full h-64 bg-gradient-to-br from-purple-100 to-pink-100 flex items-center justify-center">
                          <span className="text-purple-600">Media Preview</span>
                        </div>
                      )}
                    </div>
                  )}
